  ...
  return <>
    <div className="app-shell">
      <aside className="sidebar" aria-label="Primary navigation">
        ...
        <div className="sidebar-label">Browse by month</div><nav className="product-list" aria-label="Months">{releases.map((document) => <a className="product-link" key={document.month.value} href={`#${document.month.value}`}><span className="product-dot" />{document.month.label}</a>)}</nav>
      ...
      <main id="top" className="main-content">
        ...
        <section id="updates" className="updates-section month-documents" aria-labelledby="updatesTitle"><div className="section-heading"><div><p className="eyebrow">UiPath release notes</p><h2 id="updatesTitle">2026 monthly documents</h2></div><span className="result-count">{String(releases.length).padStart(2, "0")} months</span></div>
          {loading && <div className="loading-state">Loading release notes from UiPath...</div>}{error && !loading && <div className="loading-state">{error}</div>}{!loading && !error && <div className="month-document-list">{releases.map((document) => <article className="month-document" id={document.month.value} key={document.month.value}><div className="month-document-heading"><p className="eyebrow">Cloud platform / 2026</p><h3>{document.month.label} 2026</h3></div><div className="release-note-document" dangerouslySetInnerHTML={{ __html: sanitizeNoteContent(document.content) }} /></article>)}</div>}
        </section>
